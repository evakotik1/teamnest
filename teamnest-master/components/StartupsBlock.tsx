...
const startupsData: StartupType[] = [
    {   
        id: 1,
        title: "Название проекта",
        description: "Lorem ipsum dolor sit amet consectetur. Est pretium urna ut dui quis at turpis id. Tincidunt cursus sodales elit egestas vestibulum eget...",
        stage: "Идея",
        startDate: "07.08.2025",
        category: "IT направление",
        mobileImage: "/MobilkaNotion.svg",
        desktopImage: "/IMG.svg"
    },
    {   
        id: 2,
        title: "Название проекта",
...
export default function StartupBlock() {
    ...
    const openModal = (startup: StartupType) => { 
    ...
    const renderStartupCard = (startup: StartupType) => (
        <>
            <div 
                key={`mobile-${startup.id}`}
                onClick={() => openModal(startup)} 
                className='block md:hidden relative flex flex-col items-start gap-4 bg-[#F8F9FB] border border-[#ECEDEE] rounded-[16px] w-full cursor-pointer'
            >
                <div className='relative w-full'>
                    <Image 
                        src={startup.mobileImage} 
                        alt="img" 
                        width={400} 
                        height={200}
                    ...
                    <div className='absolute top-3 right-3 flex flex-row justify-center items-center py-1 px-3 gap-0.5 bg-[#1C1C1C]/25 backdrop-blur-[5px] rounded-[16px]'>
                        <span className='text-sm text-white'>{startup.category}</span>
                    </div>
                </div>
                ...
                <div className='flex flex-col items-start gap-2 w-full'>
                    <div className="flex justify-between items-center gap-4 pt-2 px-4 w-full">
                        <div className='flex items-center gap-2'>
                            <h1 className='font-medium text-xl'>{startup.title}</h1>
                        </div>
                        <StarButton />
                    </div>
                ...
                <div className='flex items-start gap-2 px-4 pb-4'>
                    <div className='flex flex-row justify-center items-center py-1 px-1.5 gap-1 h-[26px] bg-[#1C1C1C]/5 rounded-[16px] text-sm'>
                        <Image 
                            ...
                            height={16}
                        />
                        <span className='flex justify-center items-center py-1 gap-0.5'>{startup.startDate}</span>
                    </div>
                    <span className='flex justify-center items-center px-1.5 py-1 h-[26px] gap-0.5 bg-[#1C1C1C]/5 rounded-[16px] text-sm'>
                        {startup.stage}
                    </span>
                </div>
            </div>
            ...
            <div 
                key={`desktop-${startup.id}`}
                onClick={() => openModal(startup)} 
                className='hidden md:flex relative flex flex-col items-start pb-4 gap-4 bg-[#F8F9FB] border border-[#ECEDEE] rounded-[16px] w-full cursor-pointer'
            >
        ...
        <div className='flex flex-col items-center py-8 md:px-4 gap-8 md:gap-12 max-w-[1440px] mx-auto'>
            ...
            <div className='flex flex-col items-start gap-4 md:gap-6 w-full'>
                ...
                <div className='grid grid-cols-1 md:grid-cols-4 gap-4 md:gap-6 w-full'>
                    {startupsData.slice(0, 4).map((startup) => (
                        <div key={startup.id}>
                ...
                <div className='grid grid-cols-1 md:grid-cols-4 gap-4 md:gap-6 w-full'>
                    {startupsData.slice(4, 6).map((startup) => (
                        <div key={startup.id}>
                            {renderStartupCard(startup)}
                        </div>
                    ))}
