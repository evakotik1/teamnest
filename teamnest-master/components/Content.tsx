...
const cardsData: CardType[] = [
    {
        ...
        position: "Бухгалтер",
        description: "Lorem ipsum dolor sit amet consectetur. Est pretium urna ut dui quis at turpis id. Tincidunt cursus sodales elit egestas vestibulum eget duis ut...",
...
export default function Content() {
    ...
    const renderCard = (card: CardType) => (
        <div key={card.id} onClick={() => openModal(card)} className='flex flex-col items-start p-4 gap-4 bg-[#F8F9FB] border border-[#ECEDEE] rounded-[16px] w-full'>
            <div className="flex justify-between items-center w-full">
                <div className='flex items-center gap-2'>
                <Image src={card.avatar} alt="image" width={40} height={40} className='rounded-full'
/>
                        <div className="flex flex-col justify-center items-start">
                            <p className=" font-medium ">{card.name}</p>
                            <p className="text-sm text-[#5F6368]">{card.position}</p>
                        </div>
                </div>
                <StarButton />
            </div>
            ...
            <div className='relative w-full'>
                <span className=' text-[#5F6368] overflow-hidden'>{card.description}</span>
            ...
            <div className="w-full h-px bg-[#1C1C1C0D]"></div>
                ...
                <div className='flex flex-wrap md:flex-nowrap w-full gap-2'>
                    {card.skills.map((skill, index) => (
                        <span key={index} className='flex justify-center items-center px-1.5 py-1 gap-0.5 w-[52px] h-[26px] bg-[#1C1C1C]/5 rounded-[16px] text-sm '>
                            {skill}
                        </span>
                    ))}
                </div>
        ...
        <div className='flex flex-col items-center py-9 md:p-4 gap-12 max-w-[1440px] mx-auto'>
            ...
            <div className='flex flex-col items-start gap-6 w-full'>
                ...
                <div className='grid grid-cols-1 md:grid-cols-4 gap-6 w-full'>
                    {cardsData.slice(0, 4).map(renderCard)}
                </div>

                <div className='grid grid-cols-1 md:grid-cols-4 gap-6 w-full'>
                    {cardsData.slice(4, 8).map(renderCard)}
                </div>

                <div className='grid grid-cols-1 md:grid-cols-4 gap-6 w-full'>
                    {cardsData.slice(8, 10).map(renderCard)}
                </div>
